...
export default function App() {
  ...
  useEffect(() => {
    ...
    document.body.className = "bg-[#06080F] text-slate-100 min-h-screen flex flex-col antialiased selection:bg-purple-500 selection:text-white bg-grid-pattern relative overflow-x-hidden max-w-full";
  }, []);
  ...
  return (
    <>
      <div className="fixed top-0 left-1/4 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-purple-600/15 rounded-full blur-[120px] pointer-events-none animate-pulse"></div>
      <div className="fixed bottom-0 right-1/4 w-[350px] sm:w-[600px] h-[350px] sm:h-[600px] bg-indigo-600/10 rounded-full blur-[140px] pointer-events-none animate-pulse"></div>

      <Header />

      <main className="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 relative z-10 max-w-full">
        
        {/* Left Panel */}
        <section className="lg:col-span-8 flex flex-col gap-4 max-w-full min-w-0">
          <VideoPlayer />
        </section>

        {/* Right Panel */}
        <section className="lg:col-span-4 flex flex-col gap-4 max-w-full min-w-0">
